create or replace PACKAGE PKG_SEL_EVT_TOPICO AS
    
    -- FUNCIÓN
    FUNCTION FN_EVENTO_TOPICOS (
        PI_TIPO_TRANSACCION     INTEGER
        ,PI_FECHA_REG_INI       VARCHAR2
        ,PI_FECHA_REG_FIN       VARCHAR2
        ,PI_TOPICO_KAFKA        INTEGER
    ) RETURN USRCTRLBO.TY_TBL_EVENTO;

    -- PROCEDIMIENTO
    PROCEDURE SP_SEL_EVT_TOPICOS(
...
END PKG_SEL_EVT_TOPICO;

-------------------------------------------------------------------------------------------------

create or replace PACKAGE BODY PKG_SEL_EVT_TOPICO AS
    ...
    IS
        ...
        RS_EVT_TOPIC USRCTRLBO.TY_TBL_EVENTO := USRCTRLBO.TY_TBL_EVENTO();
    ...
    BEGIN
        ...
        BULK COLLECT INTO RS_EVT_TOPIC
        FROM
        ...
        WHERE
            ...
            AND (TOP.FI_ID_TOPICO_KAFKA = PI_TOPICO_KAFKA
                OR PI_TOPICO_KAFKA IS NULL)
    ...
    AS
        ...
        RS_EVT_TOP USRCTRLBO.TY_TBL_EVENTO := USRCTRLBO.TY_TBL_EVENTO();
    BEGIN

        RS_EVT_TOP.EXTEND();
        -- A la variable RS_EVT se le asigna el resulset que devuelve la función
        RS_EVT_TOP := FN_EVENTO_TOPICOS(PI_TIPO_TRANSACCION, PI_FECHA_REG_INI, PI_FECHA_REG_FIN, PI_TOPICO_KAFKA);
        ...
        FROM (
            ...
            FROM TABLE (RS_EVT_TOP)
        )
        WHERE
            ORD_TOPIC BETWEEN VI_INI_ROW AND VI_FIN_ROW;

        -- DELIMITACIÓN DE NÚMERO DE PÁGINAS TOTALES
        SELECT COUNT(FI_ID_TRANSACCION) INTO PO_PAG_TOTALES FROM TABLE (RS_EVT_TOP);
        ...
        RS_EVT_TOP := USRCTRLBO.TY_TBL_EVENTO();
